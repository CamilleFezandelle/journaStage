...
$titleValue = $report->getTitle() ?? '';


$titleValue = empty($errors['title']) ? (($post['title'] ?? '') ? ($post['title'] ?? '') : $report->getTitle()) : '';
$dateValue = empty($errors['date']) ? (($post['date'] ?? '') ? ($post['date'] ?? '') : $report->getDate()) : '';
$contentValue = empty($errors['content']) ? (($post['content'] ?? '') ? ($post['content'] ?? '') : $report->getContent()) : '';
  ...
  <main class="main-center">
    <div class="container login large center">
      <div class="head-with-icon">
        <div>
          <a href="./mes-comptes-rendus?id=<?= $report->getPublicId() ?>" class="button-rounded"><i class="fa-solid fa-arrow-left"></i></a>
        </div>
      ...
      <form action="./mes-comptes-rendus?id=<?= $report->getPublicId() ?>&action=edit" method="POST" class="border-top">
        ...
        <div class="input-container">
          ...
          <textarea
            ...
            required><?= htmlspecialchars($contentValue) ?></textarea>
